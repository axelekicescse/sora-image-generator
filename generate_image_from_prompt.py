...
def read_prompt(prompt_file: str = "prompt.txt") -> Optional[str]:
    """
...
def load_session(session_file: str = "session.json") -> Optional[Dict[str, Any]]:
    """
...
def main():
    ...
    with sync_playwright() as p:
        try:
            ...
            logger.info("Navigating to sora.com")
            page.goto("https://sora.com")
            
            if not sora_utils.wait_for_element(page, "textarea", timeout=30):
                logger.error("Could not find prompt input field")
                sora_utils.log_session_result(
                    logger, 
                    prompt=prompt, 
                    status="FAILURE", 
                    error="Could not find prompt input field"
                )
                browser.close()
                sys.exit(1)
